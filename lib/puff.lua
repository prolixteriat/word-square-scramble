...
local filePath = "lib/img/puffs/"

-- -----------------------------------------------------------------------------------
-- Published functions
-- -----------------------------------------------------------------------------------

function M.newPuff( params )
	local index = math.random(1, 5)
	local puff = display.newImageRect( params.g, filePath .. index .. '.png', specs[index].w, specs[index].h )
	...
	transition.to( puff, { time = math.random(200, 400), 
						   xScale = toScale, 
						   yScale = toScale, 
						   alpha = 0, 
						   onComplete = function(object) object:removeSelf() end
						 }
				 )
